...
router.post('/DirectBill', function (req, res, next) {
    ...
    if (queueItem.name == "ORDER_DETAILS") {
        ...
        var outlet_order_id = queueItem.outlet_order_id;
...
console.log("Firebase Queue: " + process.env.FIREBASE_QUEUE);
var fq='https://deft-return-187306.firebaseio.com/';//process.env.FIREBASE_QUEUE;
var queueRef = new Firebase(fq);
...
var queue = new Queue(queueRef, options, function (data, progress, resolve, reject) {
    if (typeof data.batchOrderDetail != "undefined") {
        data.batchOrderDetail.forEach(function (element) {
            queueItem = element;
            debug("Got order details from queue :" + queueItem.bill_no + "Outlet Id : " + queueItem.outlet_id, queueItem);
            performAction(queueItem, resolve, reject);
        }, this);
    } else {
        var queueItem = data;
        debug("Got order details from queue :" + queueItem.bill_no + "Outlet Id : " + queueItem.outlet_id, queueItem);
...
function performAction(queueItem, resolve, reject) {
    if (queueItem.name == "ORDER_DETAILS") {
        // process the order details
        debug("ORDER_DETAILS");
        ...
        var sides = queueItem.sides;
        var counter_code = queueItem.counter_code;
        var current_time = (typeof queueItem.current_time != "undefined") ? "'" + queueItem.current_time + "'" : "now()";
        if (queueItem.payment_mode == "Wallet") {
            var payment_method = queueItem.payment_mode;
        } else {
            var payment_method = queueItem.payment_mode.toLowerCase().split(/\s+/).join('');
        }
        var outlet_id = queueItem.outlet_id;
        var unique_random_id = queueItem.unique_Random_Id;
        var order_barcodes = queueItem.order_barcodes;
        ...
        var outlet_order_id = (typeof queueItem.outlet_order_id != "undefined") ? queueItem.outlet_order_id : null;
        ...
        if (typeof outlet_order_id != "undefined") {
            query = 'INSERT INTO sales_order (outlet_id, time, counter_code, method, mobile_num, cardholder_name, card_no,unique_random_key,is_mobile_order,userid,outlet_order_id)\
            VALUES($1, '+ current_time + ', $2, $3, $4, $5, $6,$7,$8,$9,$10) \
            RETURNING id';
            query_array = [outlet_id, counter_code, payment_method, mobile_num, cardholder_name, credit_card_no, unique_random_id, is_mobile_order, userid, outlet_order_id];
        } else {
            query = 'INSERT INTO sales_order (outlet_id, time, counter_code, method, mobile_num, cardholder_name, card_no,unique_random_key,is_mobile_order,userid)\
      VALUES($1, now(), $2, $3, $4, $5, $6,$7,$8,$9) \
            RETURNING id';
            query_array = [outlet_id, counter_code, payment_method, mobile_num, cardholder_name, credit_card_no, unique_random_id, is_mobile_order, userid];
        }
        config.query(query,
            query_array,
            function (query_err, result) {
                ...
                if (result != null && result.rows != null && result.rows.length > 0) {
                        ...
                        function (item_id, map_callback) {
                          ...
                          VALUES ($1, $2, $3, $4, get_dispense_status($3)::dispense_status,$5,$6)', [sales_order_id,
                                ...
                                redisClient.lrange(restaurant_id + "_" + outlet_id, 0, -1, function (err, data) {
                                    ...
                                    var bill_items_query = "";
                                    var bill_items_query_array = [];
                                    if (typeof outlet_order_id != "undefined") {
                                        bill_items_query = 'INSERT INTO bill_items (sales_order_id,bill_no,food_item_id,quantity,dispense_status,mobileno,is_mobile_order,serial_no,outlet_order_id)\
                        VALUES ($1, $2, $3, $4, get_dispense_status($3)::dispense_status,$5,$6,$7,$8)';
                                        bill_items_query_array = [sales_order_id, bill_no, item_id, food_details[item_id], mobile_num, is_mobile_order, serial_no, outlet_order_id]
                                    } else {
                                        bill_items_query = 'INSERT INTO bill_items (sales_order_id,bill_no,food_item_id,quantity,dispense_status,mobileno,is_mobile_order,serial_no) \
                        VALUES ($1, $2, $3, $4, get_dispense_status($3)::dispense_status,$5,$6,$7)';
                                        bill_items_query_array = [sales_order_id, bill_no, item_id, food_details[item_id], mobile_num, is_mobile_order, serial_no
                                        ];
                                    }
                                    config.query(bill_items_query, bill_items_query_array, map_callback);
                                });
                            }
                        },
                        function (map_err, map_results) {
                    ...
                    for (var barcode in barcode_dict) {
                        var item_id = getItemId(barcode);
                        var sales_order_item_query = 'INSERT INTO sales_order_items \
            VALUES ($1, $2, $3, $4)';
                        var sales_order_item_query_array = [sales_order_id, item_id, barcode_dict[barcode], barcode];
                        if (typeof outlet_order_id != "undefined") {
                            sales_order_item_query = 'INSERT INTO sales_order_items \
            VALUES ($1, $2, $3, $4,$5)';
                            sales_order_item_query_array = [sales_order_id, item_id, barcode_dict[barcode], barcode, outlet_order_id];
                        }
                        config.query(sales_order_item_query,
                            sales_order_item_query_array,
                            function (items_err, items_result) {
                                if (items_err) {
                                    console.error(items_err);
                                    debug("Sales Order Items Error :" + items_err);
        ...
        async.map(_.keys(dispense_status_data),
            function (bill_no, map_callback) {
                ...
                var OrderStatusConditions = "";
                if (dispense_status_data[bill_no] == 'pending') {
                    OrderStatusConditions = " and (dispense_status!='dispensing' and dispense_status!='delivered')";
                } else if (dispense_status_data[bill_no] == 'dispensing') {
                    OrderStatusConditions = " and (dispense_status!='delivered')";
                } else if (dispense_status_data[bill_no] == 'delivered') {
          ...
          and sales_order_id=(select max(id) \
            ...
            " + OrderStatusConditions + "", [dispense_status_data[bill_no], bill_no, outlet_id],
                    map_callback);
...
// This handler is used to refund items for an offline refund updates.
router.post('/refund_items_offline/:id', function (req, res, next) {
    var order_id = req.params.id;
    var amount = req.body.amount;
    var bill_no = req.body.bill_no;
    var mobile_num = req.body.mobile_num;
    ...
    console.log('here 1');
    console.log('************************************************');
    getorderid(order_id, outlet_order_id, function (query_err, order_id, outlet_order_id) {
        console.log('************************************************');
        console.log('here in the funciton 1', order_id, outlet_order_id);
        console.log('************************************************');
        if (query_err) {
        ...
        console.log('************************************************');
        console.log('here in the funciton 2');
        console.log('************************************************');
        config.query('INSERT INTO sales_order_payments VALUES ($1, $2, $2, \'refund\', \'modified\')', [order_id, -parseInt(amount)],
            function (query_err, result) {
                console.log('************************************************');
                console.log('here in the funciton 3');
                console.log('************************************************');
                if (query_err) {
                    console.error(query_err);
                    res.status(500).send(query_err);
                ...
                for (var barcode in item_details) {
                    config.query('INSERT INTO sales_order_items \
                            VALUES ($1, $2, $3, $4, $5)', [order_id, getItemId(barcode), -item_details[barcode], barcode, outlet_order_id],
                        function (items_err, items_result) {
                            if (items_err) {
                                console.error(query_err);
                                return;
                    ...
                    config.query('update bill_items set quantity= quantity + $1 where outlet_order_id=$2 and food_item_id = $3', [-item_details[barcode], outlet_order_id, getItemId(barcode)],
                        function (items_err, items_result) {
                            if (items_err) {
                                console.error(query_err);
                                return;
...
function getorderid(order_id, outlet_order_id, callback) {

    if (order_id == -1) {

        config.query('SELECT  s.id from sales_order s WHERE  s.outlet_order_id=$1', [outlet_order_id],
            function (query_err, result) {
                ...
                if (typeof result.rows != "undefined" && result.rowCount > 0) {
                    order_id = result.rows[0].id;

                    callback(null, order_id, outlet_order_id);
                    return;
                } else {
                    callback("order _id not found", null, null);
                }
            });
    } else {
        callback(null, order_id, outlet_order_id);
...
// This handler is used to replace items offline ( PB 25-08-17 )
router.post('/replace_items_offline/:id', function (req, res, next) {
    var order_id = req.params.id;
    var amount = req.body.amount;
    var bill_no = req.body.bill_no;
    var mobile_num = req.body.mobile_num;
    ...
    getorderid(order_id, outlet_order_id, function (query_err, order_id, outlet_order_id) {
        ...
        for (var barcode in item_details) {
          ...
          VALUES ($1, $2, $3, $4,$5)', [order_id, getItemId(barcode), -item_details[barcode], barcode, outlet_order_id],
            ...
            config.query('update bill_items set quantity= quantity + $1 where sales_order_id=$2 and food_item_id = $3', [-item_details[barcode], order_id, getItemId(barcode)],
                function (items_err, items_result) {
                    if (items_err) {
                        console.error(items_err);
                        return;
        ...
        for (var barcode in replaced_item_details) {
            config.query('INSERT INTO sales_order_items \
          VALUES ($1, $2, $3, $4,$5)', [order_id, getItemId(barcode), replaced_item_details[barcode], barcode, outlet_order_id],
                function (items_err, items_result) {
                    if (items_err) {
                        console.error(items_err);
                        return;
            ...
            config.query('INSERT INTO bill_items (sales_order_id,bill_no,food_item_id,quantity,mobileno,outlet_order_id) VALUES ($1,$2,$3,$4,$5,$6)', [order_id, bill_no, getItemId(barcode), replaced_item_details[barcode], mobile_num, outlet_order_id],
                function (items_err, items_result) {
                    if (items_err) {
                        console.error(items_err);
                        return;
...
// This logs the quantity of different kinds of supplies at sod and eod for offline ( PB 25-08-17 )
router.post('/supplies_status_offline', function (req, res, next) {
    ...
    for (var item_id in supplies) {
        config.query('INSERT INTO supplies \
      VALUES ($1, $2, $3, $4)', [item_id, supplies[item_id], time, phase],
            function (err, result) {
                if (err) {
                    console.error(err);
                    return;
    ...
    pg.connect(conString, function (err, client, done) {
        ...
        client.query("select version from outlet where id =$1",
            ...
            function (query_err, result_outlet) {
                ...
                ,fi.master_id ,r.short_name, m.start_time,m.end_time,m.name session_name \
    ...
    pg.connect(conString, function (err, client, done) {
        ...
        if (po_id == null && reconcile_items != undefined) {
            for (var item = 0; item < reconcile_items.length; item++) {
                ...
                console.log("is_reconciled_item" + is_reconciled_item);
...
router.post('/test_mode_issue_offline/:id', function (req, res, next) {
    var outlet_id = req.params.id;
    var issue_text = req.body.text;
    var issue_time = req.body.time;
    pg.connect(conString, function (err, client, done) {
        if (err) {
            handleError(client, done, res, 'error fetching client from pool' + err);
            return;
        ...
        client.query('INSERT INTO test_mode_issue (outlet_id, issue, issue_time)\
      VALUES($1, $2,$3)', [outlet_id, issue_text, issue_time],
            function (query_err, result) {
                if (query_err) {
                    handleError(client, done, res, '/test_mode_issue/:id :: error running query' + query_err);
                    return;
...
//commented by peerbits to avoid duplicate staff roaster data 
//date 28-08-17
...
//       WHERE us.user_id=gg.user_id and us.time=gg.max) gs on main.id=gs.user_id', [outlet_id],
//       function(query_err, result) {
//         if (query_err) {
...
//   });
// });
//Added by peerbits to get unique staff roaster data 
//date 28-08-17
router.get('/staff_roster/:outlet_id', function (req, res, next) {
    var outlet_id = req.params.outlet_id;
    ...
    pg.connect(conString, function (err, client, done) {
        ...
        client.query('SELECT distinct(id),full_name as username,coalesce(shift,\'shift_end\') as shift \
      FROM ( \
        select u.id, full_name \
        from atp_user u, user_group g \
        where g.name=\'outlet\' and g.target_id=$1 and u.group_id=g.id) main \
...
// Add shif timing of staff for offline ( PB 25-08-17 )
router.post('/staff_roster_offline/:outlet_id', function (req, res, next) {
    var staff_info = req.body.data;
    var outlet_id = req.params.outlet_id;
    pg.connect(conString, function (err, client, done) {
        if (err) {
        ...
        for (var i = 0; i < staff_info.length; i++) {
            var temp_data = [staff_info[i].user_id, staff_info[i].shift, staff_info[i].time];
            sending_data.push(temp_data);
        }
        // client.query('INSERT INTO atp_user_shifts VALUES($1, $2, $3 )',
        //   [staff_info.user_id, staff_info.shift,staff_info.time ],
        //   function(query_err, result) {
        //     if (query_err) {
        //       handleError(client, done, res, '/staff_roster_offline/:outlet_id :: error running query' + query_err);
        //       return;
        //     }
        //     done();
        //     res.send('success');
        // });
        // pg_format FOR BULK INSERT ( PB 25-08-17  )
        var sql_query = pg_format('INSERT INTO atp_user_shifts (user_id, shift, time) VALUES %L', sending_data);
            ...
            try {
                ...
                client.query(query_po_details, [po_id], function (query_err, result_po_details) {
                    try {
                        ...
                        res.send({
                            "po_details": result_po_details.rows
                        });
                        return;
                    }
                    catch (e) {
            ...
            try {
                ...
                client.query(query_po_details, [po_id], function (query_err, result_get_menu_bands) {
                    try {
                        ...
                        res.send({
                            "get_menu_bands": result_get_menu_bands.rows
                        });
                        return;
                    }
                    catch (e) {
...
router.post('/save_reconcile_data', function (req, res, next) {
    ...
    var request_userid = req.body.userid
    ...
    pg.connect(conString, function (err, client, done) {
        ...
        for (var item = 0; item < request_data.length; item++) {
            ...
            var reconcile_time = request_data[item].reconcile_time;
            var userid = request_data[item].userid == undefined ? request_userid : request_data[item].userid;// fix for userid  null in online outlets
        ...
        client.query(query_outlet_register, [outlet_id], function (query_err, result_sod) {
            ...
            client.query(query_reconcile_data, [outlet_id], function (query_err, result_reconcile_data) {
                try {
                    ...
                    res.send({
                        "result_reconcile_data": result_reconcile_data.rows
                    });
                    return;
                }
                catch (e) {
...
router.get('/menu_plans/:outlet_id', function (req, res, next) {
    var outlet_id = req.params.outlet_id;
    pg.connect(conString, function (err, client, done) {
        client.query('select id,start_time,end_time,name from menu_bands where outlet_id=$1', [outlet_id],
            function (query_err, result) {
                if (query_err) {
                    handleError(client, done, res, 'error running query' + query_err);
                    return;
...
router.get('/issue_resolvers', function (req, res, next) {
    pg.connect(conString, function (err, client, done) {
        if (err) {
            handleError(client, done, res, 'error fetching client from pool' + err);
            return;
        }
        client.query('SELECT * FROM issue_resolver where active = true order by resolver_name',
            [],
            function (query_err, result) {
                if (query_err) {
                    handleError(client, done, res, '/issue_resolvers/ :: error running query' + query_err);
                    return;
                }
                // releasing the connection
                done();
...
router.get('/paradise_item_list/:outlet_id', function (req, res, next) {
    ...
    pg.connect(conString, function (err, client, done) {
        ...
        client.query('select id,name,paradise_item_id,restaurant_id from food_item where active=true and outlet_id=$1',
            [outlet_id],
            function (query_err, result) {
                if (query_err) {
                    handleError(client, done, res, '/issue_resolvers/ :: error running query' + query_err);
