    ...
    redisClient.lrange(helper.dispenser_queue_node, 0, -1,
        function(q_err, q_reply) {
            ...
            if (array_status.indexOf(updated_item.status) != -1) {
                update_mongo_order_time_detials(updated_item);
            }
...
function update_mongo_order_time_detials(updated_item) {
            ...
            if (typeof result[0] != "undefined") {
                ...
                for (var index = 0; index < order_items_details.length; index++) {
                    ...
                    if (element.barcode.indexOf(item_barcodes) != -1) {
                        ...
                        OrderItemModel.update({ "_id": element._id }, {
                ...
                OrderModel.update({ "_id": order_details._id }, { "dispense_status": order_status }, {}, function (err, reply) {
                    if (err) {
                        console.log('************************************************');
...
function getOrderDetailsByOutletOrderId(search_order_item,callback){
    ...
    OrderModel.find(search_order_item, function (error, reply) {
        if (error) {
            ...
            OrderItemModel.find({ outlet_order_id: reply[0].outlet_order_id},function(error,replyitems){
...
router.post('/submit_scanned_stock', function(req, res, next) {
   ...
   async.waterfall([
        ...
        function (menu_bands_data, callback) {
            ...
            redisClient.get(helper.barcode_comparision, function (err, redis_barcode) {
                ...
                callback(null, menu_bands_data, JSON.parse(redis_barcode));
            });


        },
        function (menu_bands_data, redis_barcode, callback) { //get all po ids from the scan items
            var plcio_data = req.body.data
            // var plcio_data = matric_to_barcode;
...
console.log("element.barcode:",element.barcode);
                    barcode = get_actual_barcode(element.barcode, redis_barcode);
                } else {
...
if (barcode==undefined) return;
                    ...
                    if (typeof allitem_qty[po_id][item_id] != "undefined" && allitem_qty[po_id][item_id] >= 1) {
                        ...
                        allpodetailsscanned[po_id].push({ "item_id": item_id, "timesamp": timestamp, "barcode": barcode, "po_id": po_id });
                    }
                ...
                console.log('************************************************');
               console.log('menu_bands',menu_bands_data);
		 other_detials = {};
            ...
            callback(null, allpoids, allpodetailsscanned, allitemids, allitem_qty, other_detials, redis_barcode);
            },
        function (allpoids, allpodetailsscanned, allitemids, allitem_qty, other_detials, redis_barcode, callback) { //creating po details from the details gained
		...
		getAllItemDetails(allitemids, function(error, mainitemdetails) {
			...
			console.log('mainitemdetails',mainitemdetails);
                   // process.exit();
                    ...
                    console.log('************************************************');
                    //process.exit();
                callback(null, allpodetailsscanned, redis_barcode);
                });

            },
        function (allpodetailsscanned, redis_barcode, callback) {
                async.parallel({
                    ...
                    function(error, results) {
                        ...
                        if (typeof offline_po_details == "undefinded" || offline_po_details == null) {
                            ...
                            console.log('************************************************');
                            console.log('alloflineposids', alloflineposids);
                            console.log('************************************************');
                        ...
                        console.log('************************************************');
                        console.log('offline_po_details', offline_po_details);
                        console.log('************************************************');
                        //process.exit();
                        redisClient.set(helper.offline_po_request_node, JSON.stringify(offline_po_details), function(error, reply) {
                        callback(error, offline_po_details, po_details, redis_barcode);
                        })

                    });
            },
        function (offline_po_details, alloflineposids, redis_barcode, callback) {

                if (alloflineposids != null) {
                    for (var key in offline_po_details) {
                        if (offline_po_details.hasOwnProperty(key)) {
                            var element = offline_po_details[key];
                            if (!alloflineposids.hasOwnProperty(key)) {
                    ...
                    redisClient.set(helper.po_details_node,
                        ...
                        callback(store_po_details_err, store_po_details_reply, redis_barcode);
                        });
                } else {
                    redisClient.set(helper.po_details_node,
        ...
        function (error, data, redis_barcode) { //setting the po details in the listing provided
            if (error) {
                console.log('************************************************');
                console.log('error', error);
                console.log('************************************************');
            ...
            submit_scanned_stocks(req, res, next, redis_barcode);
        });
...
//process.exit();   
 FoodItemModel.find({ "id": { $in: allitemids } }, function(error, reply) {
        if (error) {
              // console.log('error',error);
              // process.exit();
            callback(error, null);
...
function submit_scanned_stocks(req, res, next, redis_barcode) {
        ...
        redisClient.lrange(helper.dispenser_queue_node, 0, -1,
            function(q_err, q_reply) {
                ...
                for (var i = 0; i < plcio_data.length; i++) {
                    if (isPrePrintedEnable.toLowerCase()== 'true') {
                        var barcode = get_actual_barcode(plcio_data[i]["barcode"], redis_barcode);
                    } else {
...
if (barcode==undefined ) return false;
...
function get_actual_barcode(scanned_matrix_code, redis_barcode) {
    for (var i = 0; i < redis_barcode.length; i++) {
        if (redis_barcode[i].data_matrix_code == scanned_matrix_code) {
            return redis_barcode[i].barcode;
        }
    }
}
