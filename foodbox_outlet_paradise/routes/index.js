...
router.get('/tvDisplay', function (req, res, next) {
    ...
    redisClient.lrange(helper.pending_order_queue, 0, -1, function (err, pendingOrderQueue) {
        ...
        for (var i = 0; i < pendingOrderQueue.length; i++) {
            var element = JSON.parse(pendingOrderQueue[i]);
            //   console.log(element);
...
router.get('/displayOrders', function (req, res, next) {
    ...
    redisClient.lrange(helper.pending_order_queue, 0, -1, function (err, pendingOrderQueue) {
        ...
        for (var i = 0; i < pendingOrderQueue.length; i++) {
            ...
            {
                items.push({sno:index+1,itemName:item.itemName,Quantity:item.Quantity});
            });
            orders.push({ sno: i + 1, OrderNumber: element.ParadiseOrderNumber, OrderDate: element.OrderDateTime, vendor: element.VendorName, items: items });
