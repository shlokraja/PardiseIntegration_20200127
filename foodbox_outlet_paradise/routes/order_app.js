...
router.post('/place_order', function (req, res, next) {
    ...
    for (var key in order_details) {
        num_items += parseInt(order_details[key]["count"]);
    }
    console.log("numitems:", num_items);
                        ...
                        async.parallel({
                            dispense_id: function (callback) {
                                ...
                                redisClient.incrby(helper.dispense_id_node, num_items, function (d_err, d_reply) {
                                    if (d_err) {
                                        callback("error dispense_id_node while retreiving from redis- {}".format(d_err), null);
                                        return;
                                    }
                                    callback(null, parseInt(d_reply) - num_items);
                                });
                            },
                            barcode_comparision: function (callback) {
                                redisClient.get(helper.barcode_comparision, function (err, redis_barcode) {
                                    if (err) {
                                        callback(err, null, null);
                                        debug('error while retreiving from redis- {}'.format(err), null);
                                        return;
                                    }
                                    var comparision_data = JSON.parse(redis_barcode);
                                    if (comparision_data.length > 0) {
                                        callback(null, comparision_data);
                                    } else {
                                        callback(null, null);
                                    }
                                });
                            ...
                            stock_count: function (callback) {
                                ...
                                redisClient.get(helper.stock_count_node, function (err, reply) {
                                    if (err) {
                                        callback("error  stock_count_node while retreiving from redis- {}".format(err), null);
                                        return;
                                    }
                                    callback(null, reply);
                                });
                            ...
                            num_lanes: function (callback) {
                                redisClient.get(helper.plc_config_node, function (err, reply) {
                                    if (err) {
                                        callback('error plc_config_node while retreiving from redis- {}'.format(err), null);
                                        return;
                                    }
                            ...
                            outlet_phone_no: function (callback) {
                                redisClient.get(helper.outlet_config_node, function (err, reply) {
                                    if (err) {
                                        callback('error outlet_config_node while retreiving from redis- {}'.format(err), null);
                                        return;
                                    }
                            ...
                            function (err, results) {
                                ...
                                barcode_comparision = results.barcode_comparision;
                                ...
                                for (var item_id in order_details) {
                                    for (var j = 0; j < order_details[item_id]["count"]; j++) {
                                        ...
                                        var order_stub = createOrderStub(barcode, barcode_comparision, counter_code,
                                            heating_flag, date,
                ...
                for (var key in data.order_details) {
                    if (data.order_details.hasOwnProperty(key)) {
                        ...
                        //redisClient.incrby(helper.order_item_id_node, 1, function(err, order_item_id) {
                            console.log("stock_count[key][item_details]:::",stock_count[key]["item_details"]);
                        if (stock_count[key]["item_details"].length>0) {
...
//To be checked after testing jaga 10012020
...
router.post('/fulfill_replacement/:id', function (req, res, next) {
    ...
    // Getting the no. of items in the order
    var num_items = 0;
    console.log("OrderDetails num_items:", num_items);
    for (var key in order_details) {
    ...
    async.parallel({
        ...
        checkhqreachable: function (callback) {
            ...
            var outlet_id = process.env.OUTLET_ID;
            var data = {};
            request(hq_url + UPDATE_ITEM_ISSUES_URL + outlet_id, { timeout: 1500 },
                function (error, response, body) {
                    if (error || (response && response.statusCode != 200)) {
                        callback(null, false);
                    } else {
        ...
        function (err, results) {
            ...
            for (var item_id in order_details) {
                for (var j = 0; j < order_details[item_id]["count"]; j++) {
                    ...
                    var date = getOrderStubDate();
                    var order_stub = createOrderStub(barcode, barcode_comparision, lane_no,
                        heating_flag, date,
            ...
            if (is_internet) {
                ...
                console.log('************************************************');
                console.log('************************************************');
                console.log('bill_to_print', bill_to_print);
                console.log('************************************************');
                console.log('************************************************');
                console.log('bill_no', original_bill_no);
                console.log('************************************************');
                console.log('************************************************');
...
function replace_offline(replace_data, data_replace_item_details, outlet_order_id, order_id, callback) {
    ...
    ordersearchobject = { outlet_order_id: outlet_order_id }
    searchobj = { outlet_order_id: outlet_order_id };
    ...
    for (var index = 0; index < original_barcodes.length; index++) {
        ...
        barcode_search.push({ "barcode": new RegExp(element, "i") });
    ...
    async.parallel({
        order_details: function (callback2) {
            var fields = { __v: false, _id: false, is_set_on_HQ: false };
                    ...
                    function (reply, callback3) {
                        ...
                        main_order_item_details.forEach(function (
                            ...
                            insert_order_items = {
                                ...
                                outlet_order_id: orderitems.outelt_order_id
                            };
...
function createOrderStub(barcode, barcode_comparision, lane_no,
    heating_flag, date,
    ...
    if (isPrePrintedEnable.toLowerCase() == 'true') {
        barcode = get_matrix_code(barcode, barcode_comparision);
    }
    var veg = '1';
...
function get_matrix_code(barcode, redis_barcode) {
    for (var i = 0; i < redis_barcode.length; i++) {
        if (redis_barcode[i].barcode == barcode) {
            return redis_barcode[i].data_matrix_code;
        }
    }
}
...
/*
function to get the food items data from redis and showing it as json data by peerbits
date 
*/
...
router.post("/refund_items/:order_id", function (req, res, next) {
    ...
    console.log('************************************************');
    console.log('data["item_details"]', data["food_count"]);
    console.log('************************************************');

    search_obj = { "order_id": order_id, "outlet_order_id": req.body.outlet_order_id };
...
function offline_refund_data(order_id, barcodes, data, callback) {
    ...
    searchobj = { bill_no: data["bill_no"], outlet_order_id: data["outlet_order_id"] };
    search_order_item_by_order_id_barcode(searchobj, barcodes,
        function (err, orderitems_detail) {
            ...
            subtract_ammount_deduct(ammount_deduct, deduct_food_count, { outlet_order_id: data["outlet_order_id"] },
                function (error, reply) {
                    callback(err, reply);
                }
            );
...
function search_order_item_by_order_id_barcode(searchobj, barcode, callback) {
    ...
    for (var index = 0; index < barcode.length; index++) {
        var element = { "barcode": new RegExp(barcode[index], "i") };
