...
router.post('/store_loading_issue_items', function(req, res, next) {
    ...
    internetAvailable({
        ...
        .then(function() {
            ...
            request(STORE_LOADING_ISSUE_ITEMS_URL, { timeout: 1500 },
                function(error, response, body) {
                    if (error || (response && response.statusCode != 200)) {
                        console.error('{}: {} {}'.format(hq_url, error, body));
                        if (item_id_info != null) {
        ...
        .then(function() {
            ...
            searchFoodIssueDocument(time, function(err, reply2) {
                if (reply2 != null || typeof reply2 != "undefined") {
                    ...
                    request(hq_url + GET_FOOD_ITEM_ISSUES_URL + outlet_id + '?time=' + time, { timeout: 1500 },
                        function(error, response, body) {
                            if (error || (response && response.statusCode != 200)) {
                                console.error('{}: {} {}'.format(hq_url, error, body));
                                //res.status(500).send('{}: {} {}'.format(hq_url, error, body));
...
function searchFoodIssueDocument(inserttime, callback) {
    obj = { inserttime: new RegExp(inserttime, "i") };
    console.log('##############################');
    console.log('obj', obj);
    console.log('##############################');
    ...
    FoodIssueModel.find(obj, fields, { sort: { inserttime: -1 } }, function(err, issues) {
        if (err) {
            console.log('##############################');
            console.log('error', err);
            console.log('##############################');
        ...
        .then(function() {
            searchNonFoodIssueDocument(time, function(err, reply2) {
                if (reply2 != null || typeof reply2 != "undefined") {
                    ...
                    request(hq_url + GET_NONFOOD_ITEM_ISSUES_URL + outlet_id + '?time=' + time, { maxAttempts: 5 },
                        function(error, response, body) {
                            if (error || (response && response.statusCode != 200)) {
                                console.error('{}: {} {}'.format(hq_url, error, body));
                                //res.status(500).send('{}: {} {}'.format(hq_url, error, body));
        ...
        food_item_list: function(callback) {
            internetAvailable({
                ...
                .then(function() {
                    ...
                    var outlet_id = process.env.OUTLET_ID;
                    request(hq_url + GET_FOOD_ITEM_LIST_URL + outlet_id, { timeout: 1500 },
                        function(error, response, body) {
                            console.log('************************************************');
        ...
        non_food_types: function(callback) {
            internetAvailable({
                ...
                .then(function() {
                    ...
                    request(hq_url + GET_NON_FOOD_TYPES_URL, { timeout: 1500 },
                        function(error, response, body) {
                            console.log('************************************************');
                            console.log('error', error);
                            console.log('************************************************');
    ...
    internetAvailable({
        ...
        .then(function() {
            ...
            var options = {
                ...
                body: { "barcode_details": barcode_details, "non_food_issue": non_food_issue },
                json: true,
                timeout: 1500,
...
function search_order_by_barcode(barcode, callback) {
    obj = { barcode: new RegExp(barcode) };
        ...
        .then(function() {
            ...
            search_cashdetail(today, function(error, reply) {
                ...
                if (typeof reply[0] != "undefined") {
                    ...
                    request(hq_url + AMOUNT_SOLD_CASHCARD_URL + outlet_id, { timeout: 10000 },
                        function(error, response, body) {
                            if (error || (response && response.statusCode != 200)) {
        ...
        .catch(function(err) {
            search_cashdetail("", function(error, reply) {
                ...
                if (typeof reply[0] != "undefined") {
                    ...
                    cashdetail = null;
                    saveCashDetail(JSON.stringify(cashdetail), true, function(error, reply) {
                        if (error) {
                            console.error('{}: {} {}'.format(hq_url, error, body));
                            res.status(500).send('{}: {} {}'.format(hq_url, error, body));
                            return;
...
function search_cashdetail(today, callback) {
    ...
    if (typeof today != "undefined" && today != "") {
        obj = { outlet_id: parseInt(process.env.OUTLET_ID), 'time': new RegExp(today, "i") };
    } else {
        obj = { outlet_id: parseInt(process.env.OUTLET_ID) };
    }
...
function saveCashDetail(body, set_to_HQ, callback) {
    if (body != null && typeof body != "undefined") {
        ...
        CashDetailModel.findOneAndUpdate(search_obj, cashdetail, { upsert: true, new: true }, function(error, reply) {
            if (error) {
                console.log('##############################');
                console.log('error', error);
                console.log('##############################');
    ...
    async.parallel({
            amount_sold_cash: function(callback) {
                ...
                internetAvailable({
                    ...
                    request(process.env.HQ_URL + AMOUNT_SOLD_CASH_URL, { timeout: 1500 },
                        function(error, response, body) {
                            if (error || (response && response.statusCode != 200)) {
            ...
            amount_sold_cashcard: function(callback) {
                ...
                request(process.env.HQ_URL + AMOUNT_SOLD_CASHCARD_URL, { timeout: 1500 },
                    function(error, response, body) {
                        if (error || (response && response.statusCode != 200)) {
                            // callback('{}: {} {}'.format(process.env.HQ_URL, error, body), null);
                            callback(null, { "sum": 0 });
            ...
            amount_sold_pettycash: function(callback) {
                ...
                request(process.env.HQ_URL + AMOUNT_SOLD_PETTY_CASH_URL, { timeout: 1500 },
                    function(error, response, body) {
                        if (error || (response && response.statusCode != 200)) {
                            //callback('{}: {} {}'.format(process.env.HQ_URL, error, body), null);
                            callback(null, { "sum": 0 });
        ...
        function(err, results) {
            ...
            if (typeof dataforpeetycashonlocal != "undefinded" || dataforpeetycashonlocal != null) {
                ...
                results.amount_sold_pettycash = { "sum": total };
            }
            // console.log('************************************************');
...
router.get('/get_live_pos', function(req, res, next) {
    ...
    var outlet_id = process.env.OUTLET_ID;
    request(hq_url + UPDATE_ITEM_ISSUES_URL + outlet_id, { forever: true },
        function(error, response, body) {
            if (error || (response && response.statusCode != 200)) {
                console.error('{}: {} {}'.format(hq_url, error, body));
                res.status(500).send('{}: {} {}'.format(hq_url, error, body));
...
router.get('/get_offline_pos', function(req, res, next) {
    ...
    internetAvailable({
        ...
        var outlet_id = process.env.OUTLET_ID;
        var data = {};
        request(hq_url + UPDATE_ITEM_ISSUES_URL + outlet_id, { timeout: 5000 }, function(error, response, body) {
            if (error || (response && response.statusCode != 200)) {
    ...
    redisClient.lrange(helper.reconcile_data_node, 0, -1, function(err, reply) {
		...
		if(element[0] == undefined){
			return;
                ...
                if (element[0].is_eod_done == 'n' && element[0].date == currentdate) {
...
function save_offline_reconsile_info(reconcile_items) {
    redisClient.get(helper.offline_po_request_node, function(error, offline_incomming_po) {
        ...
        for (var key in offline_incomming_po) {
            if (offline_incomming_po.hasOwnProperty(key)) {
                var element = offline_incomming_po[key];
                if (po_ids.indexOf(Number(key)) != -1) {
                    for (var index = 0; index < offline_incomming_po[key].length; index++) {
                        var element = offline_incomming_po[key][index];
                        offline_incomming_po[key][index].is_offline_reconcile_done = "y";
                        offline_incomming_po[key][index].is_offline_reconcile_done = "y";
                    }
                }
            }
...
router.get('/check_reconcile_data', function(req, res, next) {
    ...
    internetAvailable({
        ...
        .then(function() {
            ...
            request(check_reconcile_data_url, { timeout: 1500 },
                function(error, response, result) {
                    if (error || (response && response.statusCode != 200)) {
                        console.log("outlet_app.js :: " + '{}: {} {}'.format(hq_url + check_reconcile_data_url, error, result));
                        redisClient.get(helper.reconcile_check_data_node, function(err, result) {
                            ...
                            if (err) {
                                ...
                                console.log('##############################');
                                obj = {};
                                res.send({ "json_result": obj }).status(200).end();
                                return;
                            }
                            if (result != null) {
                                res.send({ "json_result": JSON.parse(result) }).status(200);
                                return;
                            } else {
                                res.send({ "json_result": obj }).status(200).end();
                                return;
...
router.post('/update_reconcile_stock_count_automatic', function(req, res, nest) {
    ...
    redisClient.get(helper.reconcile_stock_count_node,
        function(err, reply_reconcile_stock_count) {
            ...
            console.log('************************************************');
            if (reconcile_stock_count.length < 0) {
                console.log('************************************************');
                console.log('result array');
                console.log('************************************************');
            }
            console.log('************************************************');
            console.log('outlet app 2803 ');
            console.log('************************************************');
            ...
            for (var key in po_details_node_data) {
                if (po_details_node_data.hasOwnProperty(key)) {
                    if (po_details.hasOwnProperty(key)) {
                        newdata[key] = po_details_node_data[key];
                    }
                }
            }
...
//This function in cron job in app.js for every 1 mintues
function get_matrix_code() {
    var outlet_id = process.env.OUTLET_ID
    console.log("************Batch_id received in staff_roster method " + outlet_id);
    var GET_DATA_MATRIX_URL = '/food_vendor/get_data_matrix/' + outlet_id;
    console.log("GET_DATA_MATRIX_URL:",process.env.HQ_URL + GET_DATA_MATRIX_URL);
    // requesting the HQ to get the staff list
    request(process.env.HQ_URL + GET_DATA_MATRIX_URL, { forever: true },
        function(error, response, body) {
            if (error || (response && response.statusCode != 200)) {
                console.error('{}: {} {}'.format(process.env.HQ_URL, error, body));
                return;
            ...
            console.log("************data received from get_data_matrix (HQ) in staff_roster method(outlet) ");


            redisClient.set(helper.barcode_comparision, body,
...
date 18 aug 2017
*/
router.get('/show_bill_items/:order_id', function(req, res, next) {
    ...
    internetAvailable({
                ...
                if (order_items.length > 0) {
                    ...
                    requestretry({
                        ...
                        if (typeof body != "undefined") {
                            ...
                            search_order_item_by_order_id(body, order_id, is_set_on_HQ, function(error, reply2) {
                                res.send(reply2);
                            });
                        }
                    });
        ...
        .catch(function(err) {
            search_order_item_by_order_id(order_id, function(error, reply1) {
                reply1 = JSON.parse(reply1);
                maindata = [];
                for (var index = 0; index < reply1.length; index++) {
                    var element = reply1[index];
...
function saveOrderdetails(body, time, is_set_on_HQ, callback) {
    if (typeof body != "undefined") {
        ...
        for (var index = 0; index < len; index++) {
            ...
            neworder = order;
            OrderModel.findOneAndUpdate(search_order, neworder, { upsert: true, new: true }, function(error, reply) {
                if (error) {
                    console.log('##############################');
                    console.log('error', error);
                    console.log('##############################');
...
function saveOrderItemsDetails(body, order_id, is_set_on_HQ, callback) {
    if (typeof body != "undefined") {
        ...
        for (var index = 0; index < len; index++) {
            ...
            OrderItemModel.findOneAndUpdate(search_order_item, neworder_item, { upsert: true, new: true }, function(error, reply) {
                if (error) {
                    console.log('##############################');
                    console.log('error', error);
                    console.log('##############################');
        ...
        search_order_items = { order_id: parseInt(order_id) };
...
function search_order_item_by_order_id(order_id, callback) {
    obj = { order_id: parseInt(order_id) };
...
function searchNonFoodIssueDocument(time, callback) {
    ...
    NonFoodIssue.find(obj, fields, { sort: { inserttime: -1 } }, function(err, issues) {
        if (err) {
            console.log('##############################');
            console.log('error', err);
            console.log('##############################');
...
module.exports.get_matrix_code = get_matrix_code;