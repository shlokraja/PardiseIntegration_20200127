...
$("#orders").on("click", "#replace", function() {
    ...
    $("#orders .bill_item_checkbox:checked").each(function(index) {
	  ...
	  ' + dropdown_string + '\
        ...
        var jqxhr = $.post(OUTLET_URL + '/order_app/lock_item/' + item_id, { "direction": "increase", "delta_count": 1 })
            .done(function() {})
            .fail(function() {
...
$("#replace-dialog").on("change", "#replace-tab-content select", function() {
    ...
    var scope = this;
    var jqxhr = $.post(OUTLET_URL + '/order_app/lock_item/' + item_id, { "direction": "decrease", "delta_count": 1 })
        .done(function() {
            // lock the new item
            $.post(OUTLET_URL + '/order_app/lock_item/' + new_item_id, { "direction": "increase", "delta_count": 1 })
                .done(function() {
...
function openOrderItems(outlet_order_id, order_id, mobile_num, scope) {
    ...
    jqxhr = $.get(OUTLET_URL + "/outlet_app/show_bill_items/" + order_id)
        .done(function(data) {
            $("#temp_row").remove();
            $(scope).after(
