...
router.get('/getPendingOrders', function (req, res) {
    ...
    redisClient.lrange(helper.pending_delivery_queue, 0, -1, function (err, pendingDeliveryQueue) {
        ...
        for (var i = 0; i < pendingDeliveryQueue.length; i++) {
            ...
            sno = i + 1;
            orders.push({ sno: sno + 1, OrderNumber: element.LimeTrayOrderNumber, OrderDate: element.OrderDateTime, vendor: element.VendorName, status: statusText, dstatus: '0/' + total, remarks: 'Processing' });
        }
        redisClient.lrange(helper.pending_dispenser_queue, 0, -1, function (err, pendDispQueue) {
            ...
            for (var i = 0; i < pendDispQueue.length; i++) {
                ...
                orders.push({ sno: sno + i + 1, OrderNumber: element.bill_no, OrderDate: element.OrderDateTime, vendor: element.VendorName, status: statusText, dstatus: total - element.pending + '/' + total, remarks: 'Processing' });
            }
...
const GetParadiseFoodItemList = () => {
    var hq_url = process.env.HQ_URL + '/outlet/paradise_item_list/35';
    request({
        url: hq_url,
        method: "GET",
        timeout: 5000,
                    ...
                    redisClient.get(helper.Paradise_Last_LimeTray_Order_Date, function (err, reply) {
                        ...
                        if (reply == null) {
                            ...
                            tempResponse.forEach((e1) => {
                                ...
                                if (getLimeTrayDate(e1.Limetray_Order_Date, e1.Limetray_Order_Time) > new Date(reply)) {
                                    tempitem.push(e1);
...
const getItemDetailByLimeTrayId = async (limetrayno, order_details, paradise_food_item) => {
    ...
    return await new Promise((resolve, reject) => {
        ...
        var url = 'http://192.168.0.114:8000/ParadiseApp/createOrder';
            ...
            tempOrderData.items.forEach((item) => {
                ...
                if (tempOrderData.isPOGenerated == undefined) {
                    ...
                    if (result > 0) {
                        ...
                        var paradise_create_po_url = 'http://192.168.0.114:8008/fv_Preprinted/paradise_create_po'
