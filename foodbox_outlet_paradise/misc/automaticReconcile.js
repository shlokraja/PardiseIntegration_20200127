            ...
            if (err) {
                ...
                redisClient.get(helper.po_details_node, function(err, reply_po_details) {
                    if (err) {
                        ...
                        if (typeof reply_po_details == "string") {
                            reply_po_details = JSON.parse(reply_po_details);
                        }
                        newdata = {};
                        for (var key in reply_po_details) {
                            if (reply_po_details.hasOwnProperty(key)) {
                                var element = reply_po_details[key];
                        ...
                        var data = { "json_result": JSON.stringify(newdata), "reconcile_stock_count": reply_reconcile_stock_count };
			 ...
			 if (json_parsed_po_in_redis != undefined && json_parsed_po_in_redis != null) {
                            ...
                            for (var po_id in json_parsed_po_in_redis) {
                                if (typeof json_parsed_po_in_redis[po_id][0] == "undefined") {
                                    continue
                                }
                                // PO master values
...
function getItemDetails() {
    ...
    redisClient.get(helper.outlet_menu_items, function(err, reply) {
        ...
        // //console.log("****************** food_item_data:: " + JSON.stringify(food_item_data[0]));
