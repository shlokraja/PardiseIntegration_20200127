...
function startPrint(bill_dict, bill_no, date, time, savings, mobile_num, outlet_phone_no) {
    ...
    for (restaurant_id in bill_dict) {
        ...
        var gst_percent = 0;
        var selling_amt = 0;
        var sgst_percent = bill_item[0]["sgst_percent"];
        var cgst_percent = bill_item[0]["sgst_percent"];
        ...
        for (var i = 0; i < bill_item.length; i++) {
            ...
            $("#items tbody").prepend("<tr><td>" + (bill_item.length - i) + "</td><td>" + bill_item[i]["name"] + "<div class='side_order'>" + bill_item[i]["side_order"] + "</div></td>   <td>1111</td><td>" + quantity + "</td><td>" + toFixed(rate_per_item, 2) + " </td><td>" + total_gst_percent + "%</td><td>" + toFixed(rate_per_item * quantity, 2) + "</td></tr>");
